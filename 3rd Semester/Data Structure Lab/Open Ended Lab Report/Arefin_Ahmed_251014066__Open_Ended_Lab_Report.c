...
int main()
{
    ...
    if (choice == 2)
    {
        ...
        for (int i = 0; i < MAX; i++)
            strncpy(blocks[i], def[i], NAME_LEN - 1);
    ...
    while (1)
    {
        ...
        else if (cmd == 2)
        {
            ...
            fgets(line, sizeof(line), stdin);
            trim(line);
            strncpy(s, line, NAME_LEN - 1);

            printf("Enter destination block name: ");
            fgets(line, sizeof(line), stdin);
            trim(line);
            strncpy(d, line, NAME_LEN - 1);
            ...
            if (!bfs(src, dest, parent, visited))
                printf("No path found.\n");
            else
